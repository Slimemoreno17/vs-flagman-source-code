...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'fear':
				dialogue = CoolUtil.coolTextFile(Paths.txt('fear/fearDialogue'));
			case 'schizophrenia':
				dialogue = CoolUtil.coolTextFile(Paths.txt('schizophrenia/schizoDialogue'));
		}
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'fear' | 'schizophrenia': 
			{
					curStage = 'flagstreet';

					var bg:FlxSprite = new FlxSprite(-600, -200);
					bg.loadGraphic(Paths.image('flagman/FLAGBG'));
					bg.active = false;
					bg.updateHitbox();
					bg.antialiasing = true;
